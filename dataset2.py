...
batchsz = (20 + (Parameters.mixNum)) // 10
input_size = Parameters.input_size
task = Parameters.task
weather = Parameters.weather
mixNum = Parameters.mixNum

txtFile = "mix_foggy_" + str(mixNum) + "_labels.txt"

txtFile1 = "mix_foggy_10_test_labels.txt"


class PatchsTrain(Dataset):

    def __init__(self, root, tf, mode):   # mode:模式（训练 or 测试）   tf:transform对图像采取变换
        super(PatchsTrain, self).__init__()

        self.root = root
        self.tf = tf

        # image_path, label   (路径+标签)
        self.images, self.labelsP, self.labelsW = self.load_txt("data/real/" + txtFile)

        # 从上面的全部图片信息中截取不同的数量用作不同用途

        self.images = self.images[:]
        self.labelsP = self.labelsP[:]
        self.labelsW = self.labelsW[:]
...
class PatchsTest(Dataset):

    def __init__(self, root, tf, mode):   # mode:模式（训练 or 测试）   tf:transform对图像采取变换
        super(PatchsTest, self).__init__()

        self.root = root
        self.tf = tf

        # image_path, label   (路径+标签)
        self.images, self.labelsP, self.labelsW = self.load_txt("data" + "/real/" + txtFile1)
...
data_transforms ={
    'train': transforms.Compose([
        lambda x:Image.open(x).convert('RGB'),  # string path => original data (变为图像的数据类型)
...
train_datasets = PatchsTrain(data_path + "/real", data_transforms['train'], mode='train')
test_datasets = PatchsTest(data_path + "/real", data_transforms['test'], mode='test')
