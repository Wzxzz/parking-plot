...
import wandb
wandb.init()
os.environ["WANDB_API_KEY"] = '73c7c937de5aef6dab6d1531a2508aae9aebb8f5'
...
def main():
    ...
    with open("result real/" + result_file, "a", encoding="utf-8") as f:
        f.write("\n\n时间:{}\n".format(now))
    ...
    for epoch in range(epochs):
        ...
        n_iter = (epoch - 1) * len(train_loader) + batchidx + 1
        ...
        with torch.no_grad():  # 表示测试过程不需要计算梯度信息
            ...
            for x, labelP, labelW in test_loader:
                ...
                total_num += x.size(0)
                loss = criteon(logits, label)  # 损失函数
                test_loss += loss.item()  # 将每个批次的损失值累加到 test_loss 中，以计算整个测试集的平均损失。
        acc = total_correct / total_num  # 准确度
        print('Epoch:{}\t\tACCURACY:{:.2f}%'.format(epoch, acc*100))
        with open("result real/" + result_file, "a", encoding="utf-8") as f:
            f.write('{:.2f}\n'.format(acc*100))

        wandb.log({'Test loss': test_loss /  total_num, 'epoch': epoch})
        wandb.log({'Test Accuracy': total_correct / total_num, 'epoch': epoch})

        y_values.append('{:.2f}'.format(acc*100))

        # add informations to tensorboard

        writer.add_scalar('Test/Average loss', test_loss /  total_num, epoch)
        writer.add_scalar('Test/Accuracy', total_correct / total_num, epoch)
    # 保存最后一轮训练完成后的网络模型
    state = {
